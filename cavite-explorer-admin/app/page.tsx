...
import BadgeImageCropper from "./components/BadgeImageCropper";
...
export default function Home() {
  ...
  const load = async () => { if (!token) return; try { const profile = await api("/auth/me"); const role = profile.user?.role || "user"; sessionStorage.setItem("admin_role", role); sessionStorage.setItem("admin_user_id", profile.user?.id || ""); sessionStorage.setItem("admin_name", profile.user?.name || ""); sessionStorage.setItem("admin_email", profile.user?.email || ""); renderSidebarUser(profile.user || {}); const [users, places, stops, routes, tricycleTerminals, businesses, offers, overview] = await Promise.all([role === "admin" ? api("/admin/users") : Promise.resolve([]), api("/admin/places"), api("/admin/transport/stops"), api("/admin/transport/routes"), api("/admin/transport/tricycle-terminals"), api("/admin/businesses"), api("/admin/offers"), api("/admin/analytics")]); setData({ users, places, stops, routes, tricycleTerminals, businesses, offers }); setAnalytics(overview); setMessage(""); } catch (error: any) { setMessage(error.message); } };
  ...
  useEffect(() => {
    ...
    const legacyForm = Array.from(document.querySelectorAll<HTMLElement>(".entry-card")).find((card) => card.querySelector("h2")?.textContent === "Add a partner business");
...
function Overview({ count, setTab }: { count: (key: string) => number; setTab: (tab: Tab) => void }) {
  const [report, setReport] = useState<any>(null);
  const [verifying, setVerifying] = useState("");
  const loadReport = async () => {
    const response = await fetch(`${API}/admin/analytics`, { headers: { Authorization: `Bearer ${sessionStorage.getItem("admin_token") || ""}` } });
    if (response.ok) setReport(await response.json());
  };
  useEffect(() => { loadReport(); }, []);
  const verify = async (id: string) => {
    setVerifying(id);
    await fetch(`${API}/admin/transport/routes/${id}/verify`, { method: "PATCH", headers: { Authorization: `Bearer ${sessionStorage.getItem("admin_token") || ""}` } });
    await loadReport(); setVerifying("");
  };
  const totals = report?.totals || {};
  const stats = [
    ["Visitors", totals.visitors ?? 0, "Accounts"], ["Badges claimed", totals.badges ?? 0, "Verified visits"],
    ["Completed trips", totals.completedTrips ?? 0, "Journeys"], ["Redemptions", totals.redemptions ?? 0, "Partner rewards"],
  ];
  return <>
    <section className="hero"><div><p className="eyebrow">SYSTEM OVERVIEW</p><h2>Know what explorers use and what needs attention.</h2><p>Live totals come from visitor, badge, route, and partner activity.</p><button className="primary" onClick={() => setTab("transport")}>Review transport data <span>→</span></button></div><div className="hero-route"><span className="route-stop start">Visit</span><i></i><span className="route-stop transfer">Collect</span><i></i><span className="route-stop end">Redeem</span></div></section>
    <section className="stats-grid analytics-stats">{stats.map(([label, value, note]) => <article className="metric" key={label as string}><span>{label}</span><strong>{value}</strong><small>{note}</small></article>)}</section>
    <section className="overview-grid analytics-grid">
      <article className="card"><div className="section-heading"><div><p className="eyebrow">ROUTE QUALITY</p><h2>Verification reminders</h2><p>Routes should be checked every {report?.staleAfterDays || 90} days.</p></div><span className={`pill ${report?.staleRoutes?.length ? "attention" : ""}`}>{report?.staleRoutes?.length || 0} due</span></div>
        <div className="analytics-list">{report?.staleRoutes?.length ? report.staleRoutes.map((route: any) => <div key={route.id}><div><strong>{route.name}</strong><span>{route.lastVerifiedAt ? `Last checked ${new Date(route.lastVerifiedAt).toLocaleDateString()}` : "Never verified"}</span></div><button className="secondary" disabled={verifying === route.id} onClick={() => verify(route.id)}>{verifying === route.id ? "Saving…" : "Mark verified"}</button></div>) : <p className="empty">All active routes are currently verified.</p>}</div>
      </article>
      <article className="card"><p className="eyebrow">TOP LANDMARKS</p><h2>Most collected badges</h2><div className="analytics-list ranked">{(report?.topLandmarks || []).map((place: any, index: number) => <div key={place.id}><b>#{index + 1}</b><div><strong>{place.name}</strong><span>{place.badges} badges · {place.reviews} reviews</span></div></div>)}</div></article>
      <article className="card"><p className="eyebrow">PARTNER ACTIVITY</p><h2>Most redemptions</h2><div className="analytics-list ranked">{(report?.topPartners || []).map((partner: any, index: number) => <div key={partner.id}><b>#{index + 1}</b><div><strong>{partner.name}</strong><span>{partner.redemptions} redemptions</span></div></div>)}</div><p className="activity-note">{totals.recentRedemptions || 0} redemptions in the last 30 days.</p></article>
      <article className="card"><p className="eyebrow">CURRENT COVERAGE</p><h2>Published network</h2>{[["Landmarks", totals.landmarks ?? count("places")], ["Active routes", totals.routes ?? 0], ["Approved partners", totals.partners ?? count("businesses")], ["Community reviews", totals.reviews ?? 0]].map(([label, value]) => <div className="coverage" key={label as string}><span>{label}</span><div><i style={{width:`${Math.min(Number(value) * 8, 100)}%`}} /></div><b>{value}</b></div>)}</article>
    </section>
  </>;
}
...
const badgeSymbols: Record<string, string> = { landmark: "CE", monument: "★", church: "✦", museum: "▣", nature: "❧", plaza: "◆" };

function LandmarkBadgeFields({ values }: { values?: Record<string, any> }) {
  const [badgeName, setBadgeName] = useState(values?.badgeName || "");
  const [badgeDescription, setBadgeDescription] = useState(values?.badgeDescription || "");
  const [badgeIcon, setBadgeIcon] = useState(values?.badgeIcon || "landmark");
  const [badgeColor, setBadgeColor] = useState(values?.badgeColor || "#176A50");
  const [badgeImage, setBadgeImage] = useState(values?.badgeImage || "");
  const [uploadingBadge, setUploadingBadge] = useState(false);
  const [badgeUploadError, setBadgeUploadError] = useState("");
  const [badgeCropFile, setBadgeCropFile] = useState<File | null>(null);
  const displayName = badgeName.trim() || `${values?.name || "Landmark"} Explorer`;
  const chooseBadgeImage = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    setBadgeUploadError("");
    setBadgeCropFile(file);
  };
  const uploadBadgeImage = async (croppedImage: Blob) => {
    setBadgeCropFile(null);
    setUploadingBadge(true);
    setBadgeUploadError("");
    try {
      const body = new FormData();
      body.append("file", croppedImage, "badge.jpg");
      const response = await fetch(`${API}/admin/places/upload-image`, { method: "POST", headers: { Authorization: `Bearer ${sessionStorage.getItem("admin_token") || ""}` }, body });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw Error(result.message || "Badge image upload failed.");
      setBadgeImage(result.path);
    } catch (uploadError: any) { setBadgeUploadError(uploadError.message); }
    finally { setUploadingBadge(false); }
  };
  return <div className="badge-designer">
    {badgeCropFile && <BadgeImageCropper file={badgeCropFile} onCancel={() => setBadgeCropFile(null)} onCropped={uploadBadgeImage} />}
    <aside className="badge-preview" style={{ "--badge-color": badgeColor } as CSSProperties}>
      <span className={`badge-preview-ring${badgeImage ? " has-image" : ""}`}>{badgeImage ? <img src={badgeImage.startsWith("http") ? badgeImage : `${API}${badgeImage}`} alt={`${displayName} badge`} /> : <b>{badgeSymbols[badgeIcon] || badgeSymbols.landmark}</b>}</span>
      <small>LANDMARK BADGE</small><strong>{displayName}</strong>
      <p>{badgeDescription.trim() || "Complete a verified visit to unlock this badge."}</p>
      <em>Locked until visited</em>
    </aside>
    <div className="badge-fields">
      <div className="badge-image-field wide-field"><input type="hidden" name="badgeImage" value={badgeImage} readOnly /><div><strong>Badge artwork</strong><span>Upload a JPG, PNG, or WebP image, then crop and position it before saving.</span></div><div className="badge-image-actions"><label className="badge-upload-button">{uploadingBadge ? "Uploading…" : badgeImage ? "Replace image" : "Upload and crop"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseBadgeImage} disabled={uploadingBadge} /></label>{badgeImage && <button type="button" className="badge-remove-button" onClick={() => setBadgeImage("")}>Remove</button>}</div>{badgeUploadError && <p>{badgeUploadError}</p>}</div>
      <label>Badge name<input name="badgeName" value={badgeName} onChange={(event) => setBadgeName(event.target.value)} placeholder="Automatically uses the landmark name" /></label>
      <label>Badge symbol<select name="badgeIcon" value={badgeIcon} onChange={(event) => setBadgeIcon(event.target.value)}><option value="landmark">Cavite Explorer</option><option value="monument">Monument or shrine</option><option value="church">Church or religious site</option><option value="museum">Museum</option><option value="nature">Park or nature</option><option value="plaza">Plaza</option></select></label>
      <label className="wide-field">Badge description<textarea name="badgeDescription" rows={3} value={badgeDescription} onChange={(event) => setBadgeDescription(event.target.value)} placeholder="What makes earning this badge meaningful?" /></label>
      <label>Badge color<div className="badge-color-field"><input name="badgeColor" type="color" value={badgeColor} onChange={(event) => setBadgeColor(event.target.value)} /><span>{badgeColor.toUpperCase()}</span></div></label>
      <label>Required visit time (minutes)<input name="badgeRequiredMinutes" type="number" min="1" max="240" defaultValue={values?.badgeRequiredMinutes ?? 30} /></label>
      <label>Verification radius (meters)<input name="badgeRadiusMeters" type="number" min="20" max="1000" step="10" defaultValue={values?.badgeRadiusMeters ?? 100} /></label>
    </div>
  </div>;
}

const landmarkWizardSteps = ["Essential", "Visitor", "History", "Reminders", "Badge", "Review"];
...
function Form({ title, subtitle, fields, save }: { title: string; subtitle: string; fields: string[]; save: (event: FormEvent<HTMLFormElement>) => void }) { const isLandmark = title === "Add a landmark"; const isPartner = title === "Add a partner business"; const effectiveFields = isPartner ? ["name", "category", "address", "municipality", "barangay", "latitude", "longitude", "contact", "operatingHours", "partnerEmail", "description", "image"] : fields; const hasLocation = effectiveFields.includes("latitude") && effectiveFields.includes("longitude"); if (isLandmark) return <><LandmarkViewTabs /><LandmarkCreationWizard save={save} /></>; return <section className="card entry-card"><div className="section-heading"><div><p className="eyebrow">NEW RECORD</p><h2>{title}</h2><p>{isPartner ? "Pin the business, connect its mobile partner account, and add the reward location details." : subtitle}</p></div></div>{hasLocation && <MapPicker label={isPartner ? "Pin the partner location" : "Pin the location"} helper={isPartner ? "Search or click the exact business entrance. The partner can be routed here like a landmark." : undefined} />}<form onSubmit={save}>{effectiveFields.map((field) => <label key={field}>{field.replace(/([A-Z])/g, " $1").replace(/^./, (letter) => letter.toUpperCase())}<input name={field} type={field === "partnerEmail" ? "email" : "text"} placeholder={field === "partnerEmail" ? "Registered mobile account email" : `Enter ${field.replace(/([A-Z])/g, " $1")}`} required={["name", "category", "address", "municipality", "latitude", "longitude", "partnerEmail"].includes(field)} /></label>)}<button className="primary form-submit">Save record →</button></form></section>; }
...
function Table({ title, rows, fields }: { title: string; rows: any[]; fields: string[] }) {
  ...
  const reviewPartner = async (row: any, decision: "approve" | "changes" | "reject") => {
    const reason = decision === "approve" ? "" : window.prompt(decision === "changes" ? "What should the partner update?" : "Why is this application rejected?") || "";
    if (decision !== "approve" && !reason.trim()) return;
    setNotice("");
    const response = await fetch(`${API}/admin/businesses/${row.id}/approval`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${sessionStorage.getItem("admin_token") || ""}` }, body: JSON.stringify({ decision, reason }) });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) { setNotice(body.message || "Could not review this partner application."); return; }
    setDisplayRows((current) => current.map((item) => item.id === row.id ? { ...item, ...body } : item));
    setNotice(decision === "approve" ? `${row.name} is approved and now visible to explorers.` : `Partner application marked as ${decision === "changes" ? "changes requested" : "rejected"}.`);
  };
  if (isPartnerDirectory) return <section className="card table-card partner-approval-directory"><div className="section-heading"><div><p className="eyebrow">PARTNER APPROVAL</p><h2>Business applications</h2><p>Review profiles submitted from partner phones. Only approved businesses appear to explorers.</p></div><span className="record-count">{displayRows.length} applications</span></div>{notice && <p className="form-message success-message">{notice}</p>}<div className="scroll"><table className="partner-review-table"><thead><tr><th>Business / account</th><th>Location</th><th>Proposed reward</th><th>Status</th><th>Review</th></tr></thead><tbody>{displayRows.length ? displayRows.map((row) => <tr key={row.id}><td><div className="partner-review-name"><strong>{row.name || "Incomplete profile"}</strong><small>{row.owner?.email || "No linked partner account"}</small><span>{row.category || "Category not set"}</span></div></td><td><strong>{[row.barangay, row.municipality].filter(Boolean).join(", ") || "Not pinned"}</strong><small className="partner-review-address">{row.address || "Address not provided"}</small></td><td><strong>{row.proposedDiscountLabel || "No discount submitted"}</strong><small className="partner-review-address">{row.proposedDiscountTitle || ""}</small></td><td><span className={`partner-status ${row.approvalStatus || "draft"}`}>{String(row.approvalStatus || "draft").replaceAll("_", " ")}</span>{row.rejectionReason && <small className="partner-review-address">{row.rejectionReason}</small>}</td><td>{isAdmin ? <div className="partner-review-actions"><button type="button" className="primary" disabled={row.approvalStatus === "approved"} onClick={() => reviewPartner(row, "approve")}>Approve</button><button type="button" className="secondary" onClick={() => reviewPartner(row, "changes")}>Request changes</button><button type="button" className="table-delete-button" onClick={() => reviewPartner(row, "reject")}>Reject</button></div> : <span>Admin review only</span>}</td></tr>) : <tr><td className="empty" colSpan={5}>No partner applications yet. Invite a partner from Accounts.</td></tr>}</tbody></table></div></section>;
...
function InvitePanel({ api }: { api: (path: string, opts?: RequestInit) => Promise<any> }) {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (sending) return;
    const form = event.currentTarget;
    setSending(true); setFailed(false); setNotice(""); setLink(""); setCopied(false);
    try {
      const payload = Object.fromEntries(new FormData(form));
      const selectedRole = String(payload.role || "editor");
      const result = await api("/admin/invites", { method: "POST", body: JSON.stringify(payload) });
      const acceptedRecipient = result.delivery?.accepted?.[0];
      setLink(result.inviteUrl);
      setNotice(acceptedRecipient ? `${selectedRole[0].toUpperCase()}${selectedRole.slice(1)} invitation accepted for ${acceptedRecipient}` : "Invitation sent successfully");
      form.reset();
      const roleSelect = form.elements.namedItem("role") as HTMLSelectElement | null;
      if (roleSelect) roleSelect.value = selectedRole;
    } catch (error: any) {
      setFailed(true); setNotice(error.message || "Could not send the invitation.");
  ...
  return <section className="card entry-card invite-panel">
    <div className="invite-heading"><div className="invite-heading-icon">+</div><div><p className="eyebrow">TEAM ACCESS</p><h2>Invite a team member</h2><p>Send secure access to an editor, administrator, or mobile partner.</p></div></div>
    <form className="invite-form" onSubmit={create}>
      <label>Name <span>(optional)</span><input name="name" placeholder="Full name" disabled={sending}/></label>
      <label>Email<input name="email" type="email" required placeholder="name@example.com" disabled={sending}/></label>
      <label>Access role<select name="role" defaultValue="editor" disabled={sending}><option value="editor">Editor — content access</option><option value="admin">Admin — full access</option><option value="partner">Partner — mobile only</option></select></label>
      <button className="primary form-submit invite-submit" disabled={sending} aria-busy={sending}>{sending && <span className="button-spinner" aria-hidden="true"/>}{sending ? "Sending invitation…" : "Send invitation"}</button>
    </form>
    {notice && <div className={`invite-notice ${failed ? "error" : "success"}`} role="status" aria-live="polite"><span className="invite-notice-icon">{failed ? "!" : "✓"}</span><div><strong>{notice}</strong>{!failed && <small>The recipient can use the secure link for the next 7 days.</small>}</div></div>}
    {link && <div className="invite-link-result"><div><span>Secure invitation link</span><code>{link}</code></div><button type="button" onClick={copyLink}>{copied ? "Copied ✓" : "Copy link"}</button></div>}
  </section>;
}
function Users({ rows, api, reload }: { rows: any[]; api: (path: string, opts?: RequestInit) => Promise<any>; reload: () => void }) {
  ...
  useEffect(() => {
    const addPartnerOption = (select: HTMLSelectElement, label: string) => {
      if (Array.from(select.options).some((option) => option.value === "partner")) return;
      const option = document.createElement("option"); option.value = "partner"; option.textContent = label; select.append(option);
    };
    document.querySelectorAll<HTMLSelectElement>(".accounts-table tbody select:not(.status-select)").forEach((select) => addPartnerOption(select, "Partner"));
    const filter = document.querySelector<HTMLSelectElement>(".account-tools select");
    if (filter) addPartnerOption(filter, "Partners");
