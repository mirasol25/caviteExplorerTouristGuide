...
export class AdminController {
  constructor(private readonly admin: AdminService, private readonly storage: CloudinaryStorageService) {}
  ...
  async uploadPlaceImage(@UploadedFile() file?: Express.Multer.File) {
    ...
    return { path: await this.storage.upload(file, 'landmarks') };
  }
  ...
  @Patch('transport/routes/:id/verify') verifyRoute(@Req() req: any, @Param('id') id: string) { return this.admin.verifyRoute(req.user.id, id); }
  ...
  @Patch('businesses/:id/approval') reviewBusiness(@Req() req: any, @Param('id') id: string, @Body() body: { decision: string; reason?: string }) { this.requireAdmin(req); return this.admin.reviewBusiness(req.user.id, id, body); }
